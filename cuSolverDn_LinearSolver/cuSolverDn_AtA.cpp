...
int main (int argc, char *argv[])
{
    ...
    // if (loadMMSparseMatrix<float>(opts.sparse_mat_filename, 'd', true , &rowsA, &colsA,
    ...
    cbstat = cublasgemm<float>(cublasHandle,CUBLAS_OP_T,CUBLAS_OP_N,colsA,colsA,rowsA,&al,d_A,rowsA,d_A,rowsA,&bet,dAtA,colsA);
    ...
    cbstat = cublasgemv<float>(cublasHandle,CUBLAS_OP_T,colsA,colsA,&al,d_A,colsA,d_b,1,&bet,d_Atb,1);
    ...
    {
        linearSolverSVD<float>(handle, colsA, dAtA, colsA, d_Atb, d_x);
    }
    else if ( 0 == strcmp<float>(opts.testFunc, "chol") )
    {
        linearSolverCHOL(handle, colsA, dAtA, colsA, d_Atb, d_x);
    }
    else if ( 0 == strcmp<float>(opts.testFunc, "lu") )
    {
        linearSolverLU<float>(handle, colsA, dAtA, colsA, d_Atb, d_x);
    }
    else if ( 0 == strcmp(opts.testFunc, "qr") )
    {
        linearSolverQR<float>(handle, colsA, dAtA, colsA, d_Atb, d_x);
    }
    else
    {
