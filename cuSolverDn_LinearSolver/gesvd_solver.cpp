...
{ 
	...
	const int m = 3; const int n = 3; const int lda = m; 
	/*     | 1 2 | 
	 * A = | 4 5 | 
	 *     | 2 1 | 
	 */ 
	double A[lda*n] = { 1.0, 4.0, 2.0, 2.0, 5.0, 1.0, 0., 0., 0.}; 
	double U[lda*m]; // m-by-m unitary matrix 
	double VT[lda*n]; // n-by-n unitary matrix
	double S[n]; //singular value 
	double S_exact[n] = {7.065283497082729, 1.040081297712078, 0.}; 
	double *d_A = NULL; double *d_S = NULL; 
	double *d_U = NULL; double *d_VT = NULL; 
	int *devInfo = NULL; double *d_work = NULL; 
	double *d_rwork = NULL; double *d_W = NULL; 
	// W = S*VT
	int lwork = 0; int info_gpu = 0;
	...
	//lda is ldu; second lda isldvt
	cusolver_status = cusolverDnDgesvd ( 
		cusolverH, jobu, jobvt, m, n, d_A, lda, d_S, d_U, lda, d_VT, lda, d_work, lwork, d_rwork, devInfo);
	...
	assert(cudaSuccess == cudaStat3); assert(cudaSuccess == cudaStat4); 
	...
	printf("|S - S_exact| = %E \n", ds_sup); 
	// step 6: |A - U*S*VT| // W = S*VT 
	cublas_status = cublasDdgmm( cublasH, CUBLAS_SIDE_LEFT, n, n, d_VT, lda, d_S, 1, d_W, lda); 
	assert(CUBLAS_STATUS_SUCCESS == cublas_status); 
...
// Read more at: http://docs.nvidia.com/cuda/cusolver/index.html#ixzz4ZTHTNOoi 
